...
exports.download = async (req, res, next) => {
  try {
    ...
    // Serve a previously uploaded PDF from Netlify Blobs when available.
    if (payslip.fichier_pdf) {
      const stored = await getPdf(payslip.fichier_pdf);
      if (stored) {
    ...
    // Otherwise generate a print-ready HTML bulletin. The frontend recognises
    // the text/html content type and saves it with an .html extension.
    ...
    res.setHeader('Content-Disposition', `inline; filename="${(payslip.fichier_pdf || 'bulletin').replace(/\.pdf$/i, '')}.html"`);
    return res.end(html);
  } catch (err) {
